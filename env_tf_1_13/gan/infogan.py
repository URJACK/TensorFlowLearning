...
def discriminator(data, scope="discriminator", reuse=None, latent=False):
...
def discriminator_loss(logics_real, logics_fake, labels_real, label_placeholder, scope='D_Loss'):
    with tf.variable_scope(scope):
        loss = tf.losses.log_loss(true_labels, tf.sigmoid(logics_real)) + tf.losses.log_loss(fake_labels, tf.sigmoid(
            logics_fake)) + tf.losses.softmax_cross_entropy(logits=labels_real,
                                                            onehot_labels=label_placeholder)
        return loss


def generator_loss(logics_fake, labels_fake, label_placeholder, scope='G_loss'):  # 生成网络的`loss`
    with tf.variable_scope(scope):
        loss = tf.losses.log_loss(true_labels, tf.sigmoid(logics_fake)) + tf.losses.softmax_cross_entropy(
            logits=labels_fake, onehot_labels=label_placeholder)
        return loss
...
# 噪声和label共同拼接的输入信号
g_input = tf.concat([sample_noise, label_ph], 1)

# 生成的假图片
inputs_fake = generator(g_input)

logits_real = discriminator(dc_inputs)
logits_fake = discriminator(inputs_fake, reuse=True)
labels_real_dis = discriminator(dc_inputs, reuse=tf.AUTO_REUSE, latent=True)
labels_fake_dis = discriminator(inputs_fake, reuse=True, latent=True)

d_total_error = discriminator_loss(logits_real, logits_fake, labels_real_dis, label_ph)
g_total_error = generator_loss(logits_fake, labels_fake_dis, label_ph)
...
saver.save(sess, os.path.abspath(os.path.dirname(__file__)) + "\\model\\infogan.ckpt")
